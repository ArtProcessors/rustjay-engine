...
fn choose_config(
    ...
    for tier_max in (0..tiers.len()).rev() {
        ...
        // Build attribute list, merging SURFACE_TYPE bits from all tiers to avoid
        // duplicate SURFACE_TYPE entries (undefined behaviour in EGL; Mesa returns
        // no configs when the same attribute key appears twice in the list).
        attributes.clear();
        let mut surface_bits = 0i32;
        let mut non_surface: Vec<i32> = Vec::new();
