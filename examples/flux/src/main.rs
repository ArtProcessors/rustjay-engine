...
impl PingPong {
    ...
    fn read(&self) -> &Texture {
        &self.textures[self.read]
    }
    fn write(&self) -> &Texture {
        &self.textures[1 - self.read]
    }
    fn swap(&mut self) {
...
impl EffectPlugin for FluxEffect {
    ...
    fn default_state(&self) -> FluxState {
        FluxState::default()
    }
    ...
    fn render(&mut self, ctx: &mut RenderHookCtx<'_>, app_state: &mut Self::State) -> bool {
        ...
        let Some(input_tex) = input.texture else {
            return false;
        };
        ...
        let Some(ubg) = &self.uniform_bg else {
            return false;
        };
        let Some(flow_pl) = &self.flow_pipeline else {
            return false;
        };
        let Some(warp_pl) = &self.warp_pipeline else {
            return false;
        };
        let Some(blit_pl) = &self.blit_pipeline else {
            return false;
        };
        let Some(flow_bgl) = &self.flow_bgl else {
            return false;
        };
        let Some(warp_bgl) = &self.warp_bgl else {
            return false;
        };
        let Some(blit_bgl) = &self.blit_bgl else {
            return false;
        };
        ...
        let flow_bg = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::TextureView(&prev_frame.view),
                },
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: wgpu::BindingResource::TextureView(&flow.read().view),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
                    resource: wgpu::BindingResource::Sampler(input_samp),
                },
            ],
        });
        {
        ...
        let warp_bg = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::TextureView(&flow.write().view),
                },
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: wgpu::BindingResource::TextureView(&accum.read().view),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
                    resource: wgpu::BindingResource::Sampler(input_samp),
        ...
        {
            let mut pass =
                Self::fullscreen_pass(ctx.encoder, &accum.write().view, "Flux Warp Pass");
        ...
        let blit_bg = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&accum.write().view),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::Sampler(input_samp),
...
impl FluxEffect {
    fn build_uniforms_static(s: &FluxState, e: &EngineState) -> FluxUniforms {
        let audio_reactive = e
            .get_param("audio_reactive")
            .map(|v| v > 0.5)
...
impl AnyGuiTab for FluxTab {
    ...
    fn draw(
        ...
        if ui
            .slider_config("Flow Scale", 0.1_f32, 5.0)
            .build(&mut s.flow_scale)
        {
            engine.set_param_base("flow_scale", s.flow_scale);
        }
        if ui
            .slider_config("Smoothing", 0.0_f32, 0.95)
            .build(&mut s.flow_smooth)
        {
            engine.set_param_base("flow_smooth", s.flow_smooth);
        }
        if ui
            .slider_config("Lambda", 0.001_f32, 0.1)
            .build(&mut s.flow_lambda)
        {
        ...
        if ui
            .slider_config("Warp", 0.0_f32, 2.0)
            .build(&mut s.warp_strength)
        {
            engine.set_param_base("warp_strength", s.warp_strength);
        }
        if ui
            .slider_config("Drift", 0.0_f32, 0.5)
            .build(&mut s.drift_strength)
        {
            engine.set_param_base("drift_strength", s.drift_strength);
        }
        if ui
            .slider_config("Feedback", 0.8_f32, 0.999)
            .build(&mut s.feedback_decay)
        {
            engine.set_param_base("feedback_decay", s.feedback_decay);
        }
        if ui
            .slider_config("Webcam Mix", 0.0_f32, 1.0)
            .build(&mut s.webcam_mix)
        {
        ...
        if ui
            .slider_config("Flow Viz", 0.0_f32, 1.0)
            .build(&mut s.flow_viz)
        {
            engine.set_param_base("flow_viz", s.flow_viz);
        }
        if ui
            .slider_config("Viz Scale", 0.5_f32, 20.0)
            .build(&mut s.flow_viz_scale)
        {
...
fn main() -> anyhow::Result<()> {
    ...
    #[cfg_attr(
        not(any(feature = "gles2", feature = "drm-gles2")),
    ...
    if nogui && gles2 {
        #[cfg(feature = "gles2")]
        {
            ...
            return rustjay_engine::run_gles2_headless_with_tabs(FluxEffect::default(), renderer);
        }
